...
function HeatCapacity_FrequencySweep_NoTimeConstantAdjustment(DAQ)
...
%     pause(20);
    for j=1:PointsPerFrequency
        ...
        % Calculations.
        datacell.Heater_LI_Sensitivity(ii) = NaN;
        datacell.Heater_LI_Timeconstant(ii) = LI_TimeConstant;
        datacell.Thermometer_LI_Sensitivity(ii) = NaN;
        ...
        if mod(ii,100)==1
            ...
            xlabel('Time [s]'); ylabel('R_{CX}.*Gain.*f [\muV\cdotHz]'); 
