...
function HeatCapacity_AmplitudeSweep_CS580(DAQ)
...
AmplitudeSweepVector = DAQ.AmplitudeSweepVector;
PointsPerAmplitude = DAQ.PointsPerAmplitude;
...
datacell.PointsPerFrequency = PointsPerAmplitude; 
datacell.SetHeaterFrequency = DAQ.SetHeaterFrequency;
SetHeaterFrequency = datacell.SetHeaterFrequency; 

K6221_WaveMode_SetAmplitude(K6221_Heater,AmplitudeSweepVector(1));
K6221_WaveMode_SetFrequency(K6221_Heater,SetHeaterFrequency);
K6221_WaveMode_OnOff(K6221_Heater,'On');
LI_TimeConstant = SRS830_FindTimeConstant(SetHeaterFrequency);
...
DataFigure = figure;
MyClock = tic;
for i=1:length(AmplitudeSweepVector)
    SetFrequency = SetHeaterFrequency;
    SetAmplitude = AmplitudeSweepVector(i);
...
%     pause(20);
    for j=1:PointsPerAmplitude
        ...
        % Calculations.
        datacell.Heater_LI_Sensitivity(ii) = 0;
        datacell.Heater_LI_Timeconstant(ii) = LI_TimeConstant;
        datacell.Thermometer_LI_Sensitivity(ii) = 0;
        ...
        if mod(ii,100)==1
            ...
            xlabel('Time [s]'); ylabel('V_{CX}.*Gain [\muV]'); 
            
            subplot(2,2,3:4); hold off; 
            plot(datacell.HeaterAmplitude.^2,datacell.R_Thermometer.*1e6./LI_Gain,'-ok'); 
            xlabel('f [Hz]'); ylabel('V_{CX} [\muV]'); 
...
filename = [DAQ.Aswpfileroot,'\',regexprep([DAQ.SampleInfoStr,'_',...
