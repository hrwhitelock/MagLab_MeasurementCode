function ThermCondHallMeasureOnePoint_v3(DAQ)
...
currentTemp = read336(gpib_ls336_1,ProbeChannel,K_unit);
...
ii =0; tic;
 ...
 obj_ls336_1 =  open_ls336(gpib_ls336_1);
%obj_ls336_1 =  open_ls336(gpib_ls336_1);

datacell.SorbHtrPower = read336_misc(obj_ls336_1,'HTR? 1');
datacell.ProbeHtrPower = read336_misc(obj_ls336_1,'HTR? 2');


%display(datacell.SorbHtrPower);
%     s = {'A','B','C','D'};
%     for i = 1:4
%         datacell.(['Input' s{i}]) = read336_misc(obj_ls336_1, ['INTYPE? ' s{i}]);
...
for currentCurrent = I
    ...
    for k = 1:SamplesPerStep
        ...
        datacell.BathRes(ii)=read336_obj(obj_ls336_1,BathChannel,R_unit);
        datacell.BathTemp(ii)=CXcell{4}(datacell.BathRes(ii));
        datacell.VTITemp(ii)=read336_obj(obj_ls336_1,VTIChannel,K_unit);
        %datacell.He3Temp(ii)=read336(gpib_ls336_2,He3VTIChannel,K_unit);
        datacell.ProbeTemp(ii)=read336_obj(obj_ls336_1,ProbeChannel,K_unit);
        ...
        %datacell.HotTempLs(ii)=read336(gpib_ls336_1,HotChannel,K_unit);
        %datacell.ColdTempLs(ii)=read336(gpib_ls336_1,ColdChannel,K_unit);
