function one_cycle_thermHall_SCM7(filename, cycleNum, current, DAQ)
...
while ~cycle_completed
	...
	if mod(ii, 2) == 0
    	...
    	subplot(3,2,1);
    	plot(datacell.Time,datacell.hotTemp-datacell.coldTemp,'-c.'); grid on; box on;hold on; 
        ...
        hold on; 
        yyaxis left; 
        plot(datacell.Time,datacell.bathRes,'-b.'); grid on; box on;
        yyaxis right; 
...
%         plot(datacell.Time, datacell.ChanD, '--g')
        plot(datacell.Time, datacell.hallVoltage); grid on; box on; 
