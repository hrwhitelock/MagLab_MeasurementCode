...
function HeatCapacity_AngleSweep(DAQ)
...
while ishandle(myhandle)
    ...
    if mod(ii,100)==1
        ...
        plot(datacell.RotatorNumber./102,datacell.R_Thermometer.*1e6,'-k');
        xlabel('Angle [deg]'); ylabel('R_{CX} [\muV]');
        
        subplot(2,2,3); hold off;
        plot(datacell.RotatorNumber./102,datacell.R_HallSensor,'-k');
        xlabel('T_{Bath} [K]'); ylabel('R_HallSensor []');
        
        subplot(2,2,4); hold off;
        plot(datacell.RotatorNumber./102,1./datacell.TOsc_Rough,'-k');
        xlabel('Angle [deg]'); ylabel('1/T_{Osc} \propto C_S [K^{-1}]');
        drawnow;
...
filename = [DAQ.Angleswpfileroot,regexprep([DAQ.SampleInfoStr,'_',...
